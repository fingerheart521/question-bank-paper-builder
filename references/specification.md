...
“考研 408”指计算机学科专业基础综合考试，是一个综合考试项目，不应简单等同于一个不可细分的学科标签。其题库至少应区分数据结构、计算机组成原理、操作系统、计算机网络四个内容域，并在题目的 `subject` 或 `knowledge_points` 中保留该信息。组卷时应按用户要求对四个内容域设置数量或覆盖约束。
